...
void	author_line_initializing(t_vdata *vdata)
{
	...
	ft_strncpy(vdata->adv_line + (AUTHOR_LEN - 1 - ft_strlen(adv)), adv, ft_strlen(adv));

	ft_bzero(vdata->author_line, AUTHOR_LEN);
	ft_memset(vdata->author_line, ' ', AUTHOR_LEN - 1);
	ft_strncpy(vdata->author_line + (AUTHOR_LEN - 1 - ft_strlen(authors)), authors, ft_strlen(authors));
...
void	author_line_scrolling(t_vdata *vdata)
{
	...
	mvwprintw(vdata->authors, 1, 5, vdata->author_adv_switch ? vdata->adv_line : vdata->author_line);
	wrefresh(vdata->authors);
}
